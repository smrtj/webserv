...
# Deploy Embedded Assets Script
# This script replaces the external loader with an embedded version
...
# Find all index.html files and replace the old loader
find "$BASE_DIR" -type f -name "index.html" | while read -r file; do
  ...
  # Check if it already has the embedded version
  if grep -q "SMRT/KJO/HackServ Universal Asset Loader - Fully Embedded Version" "$file"; then
    echo "[SKIP] $file already has embedded version"
    continue
  fi
  
  # Check if it has the old loader reference
  if grep -q '<script src="/assets/loader.js"></script>' "$file"; then
    echo "[REPLACE] Replacing external loader in $file"
    sed -i 's|<script src="/assets/loader.js"></script>|'"${EMBEDDED_SCRIPT}"'|g' "$file"
  else
    echo "[ADD] Adding embedded loader to $file"
    # Insert before closing </body> tag, or at end if not found
    if grep -q "</body>" "$file"; then
      sed -i "s|</body>|$EMBEDDED_SCRIPT\n</body>|" "$file"
    else
      echo "$EMBEDDED_SCRIPT" >> "$file"
    fi
  fi
done

echo "✅ Embedded asset loader deployed to all pages"